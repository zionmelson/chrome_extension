chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.action === "getRecommendedVideos") {
    ...
    const searchkey =
      "f053ecf0867b93ab982aea4bf0e1177099865234d4472e78a282263ebc6456c8";

    // fetch random videos, articles, books
    Promise.allSettled([
      fetch(
        `https://www.googleapis.com/youtube/v3/search?key=${apiKey}&part=snippet&type=video&q=${query}`
      ).then((response) => response.json()),
      fetch(
        `https://newsapi.org/v2/everything?q=${query}&apiKey=${newsApiKey}`
      ).then((response) => response.json()),
      fetch(
        `https://www.googleapis.com/books/v1/volumes?q=${query}&key=${BookApi}`
      ).then((response) => response.json()),
      fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${tmdb}&query=${query}`
      ).then((response) => response.json()),
      // fetch(`https://ieeexploreapi.ieee.org/api/v1/search/articles?apikey=${ieeeApiKey}&querytext=${query}`)
      //   .then((response) => response.json())
      fetch(
        `https://serpapi.com/search.json?engine=google&q=${query}&google_domain=google.com&hl=en&api_key=${searchkey}`
      ).then((response) => response.json()),
    ])
      .then((results) => {
        ...
        const videos =
          results[0].value.items?.map((item) => {
            return {
        ...
        // top 5 books
        const books =
          results[2].value.items?.slice(0, 1).map((item) => {
            return {
        ...
        // });
        const links =
          results[4].value?.organic_results?.slice(0, 3).map((result) => {
            console.log(result);
            return {
              // title: result.title,
              url: result.link,
